...
const UploadPage = () => {
  ...
  useEffect(() => {
    ...
    // Subscribe to changes in media_clips table
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Media clips change:', payload);
  ...
  const handleSubmit = async (e) => {
    ...
    // Send all metadata as a single JSON string
    const metadata = {
      title,
      game,
      visibility,
    ...
    xhr.onload = () => {
      try {
        ...
        if (xhr.status === 200 && response.clip) {
          ...
          setTimeout(() => {
            router.push('/upload'); // Redirect after a short delay
