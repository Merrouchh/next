...
import { addGameTimeToUser } from '../../utils/api';
...
export const claimAchievement = async (supabase, userId, achievementId, points, completedAt) => {
  try {
    ...
    const { data: claimData, error: upsertError } = await supabase
      .from('user_achievements')
      .upsert({
        user_id: userId,
        achievement_id: achievementId,
...
export const addGameHours = async (supabase, userId, hours) => {
  try {
    ...
    console.log(`[AMOUNT DEBUG] Converting ${hours} hours to ${seconds} seconds`);

    // Get the user's gizmo_id
    const { data: userData, error: userError } = await supabase
      .from('users')
      .select('gizmo_id')
      .eq('id', userId)
    ...
    if (userError || !userData?.gizmo_id) {
      console.error('Error getting gizmo_id:', userError);
      return { 
        success: false, 
        error: 'Failed to get user gizmo ID',
        details: userError?.message 
      };
    }

    console.log(`[AMOUNT DEBUG] Calling API to add ${seconds} seconds (${hours} hours) to gizmo ID ${userData.gizmo_id}`);
    
    // Call the API to add time to the user
    const result = await addGameTimeToUser(userData.gizmo_id, seconds);
    
    // Log the result for debugging
    if (result.success) {
      console.log(`[AMOUNT DEBUG] SUCCESS: Added ${hours} hours (${seconds} seconds) to user ${userId} with gizmo ID ${userData.gizmo_id}`);
    } else {
      console.error(`[AMOUNT DEBUG] FAILED: Could not add ${hours} hours (${seconds} seconds) to user:`, result.error);
    }
