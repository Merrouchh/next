...
const ClipCard = ({ clip, isFullWidth, onClipUpdate }) => {
  ...
  const handleVisibilityToggle = async (newVisibility) => {
    ...
    try {
      ...
      // If we're on the discover page and clip is made private, fade out the card
      if (router.pathname === '/discover' && newVisibility === 'private') {
        setTimeout(() => {
          setClipData(null); // This will make the card disappear
        }, 300); // Match this with your animation duration
      }
      
      // If we're on the clip's page, refresh the page
      if (router.pathname === '/clip/[id]') {
